...
function applyLang(lang) {
  ...
  // data-i18n-ph attributes (placeholder)
  document.querySelectorAll('[data-i18n-ph]').forEach(el => {
    const key = el.dataset.i18nPh;
    if (T[key] !== undefined) el.placeholder = T[key];
  });
  // data-i18n-html attributes (innerHTML)
  document.querySelectorAll('[data-i18n-html]').forEach(el => {
    const key = el.dataset.i18nHtml;
    if (T[key] !== undefined) el.innerHTML = T[key];
  });

  // Page-specific translations based on current page
  const page = location.pathname.split('/').pop() || 'index.html';

  if (page === 'contact.html') {
    // Main title HTML
    const titleEl = document.querySelector('.main-title');
    if (titleEl && T.title_contact) {
      titleEl.innerHTML = T.title_contact + `<span class="sub">${T.sub_contact || 'GET IN TOUCH'}</span>`;
    }
  }
...
function injectComingSoonPopup() {
    ...
    <div id="cs-box">
      ...
      <div id="cs-inner">
        ...
        <div id="cs-actions">
          ...
          <button id="cs-close"><span id="cs-close-lbl">${T.popup_close}</span></button>
        </div>
  ...
  // Countdown to April 18, 2025
  const target = new Date('2025-04-18T00:00:00');
  function tick() {
    const now = new Date();
